<x-authentication-layout>
    ...
    <form method="POST" action="{{ route('register') }}">
        ...
        <div class="flex items-center justify-between mt-6">
            ...
            <x-button class="bg-gradient-to-r from-orange-500 to-amber-600 hover:from-orange-600 hover:to-amber-700 text-white font-bold py-2.5 px-6 rounded-xl shadow-md hover:shadow-orange-500/25 transition-all">
                {{ __('Sign Up') }}
            </x-button>                
        </div>
    ...
    <div class="pt-5 mt-6 border-t border-gray-100 dark:border-gray-700/60">
        <div class="text-sm text-gray-500">
            {{ __('Have an account?') }} <a class="font-semibold text-orange-600 hover:text-orange-700" href="{{ route('login') }}">{{ __('Sign In') }}</a>
        </div>
    </div>
</x-authentication-layout>
